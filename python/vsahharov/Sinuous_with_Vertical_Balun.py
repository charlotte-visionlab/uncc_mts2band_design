...
def width_profile_half(style, p, w_near, w_far):
    ...
    if style_lc.startswith("klopf"):
        prog = klopfenstein_progress(p, z_source_diff_ohm, z_target_antenna_diff_ohm, gamma_m=0.02)
...
def create_custom_profile_taper_trace(name_prefix, xsign, y_face, taper_style,
                                      ...
                                      num_neck_pts=14):
    """
    Single-sheet custom trace matching the user's latest reference image:
      - one continuous custom polygon, no trapezoid sections
      - long taper occupies most of the balun length (l1)
      - short bend section near the antenna-side end (l2)
      - short narrow feed transition / neck at the end (l3)

    On each balun face, the pair remains exactly parallel through the long taper,
    then bends only near the top to meet the feed transition.
    """
    total = taper_fraction + bend_fraction + neck_fraction
    ...
    z_taper_end = z3 + taper_fraction * (z_top - z3)
    z_bend_end = z_taper_end + bend_fraction * (z_top - z3)
    # Extend the final narrow section directly to the via-pad plane so there is
    # no extra rectangular "bridge" sticking above the balun substrate.
    z_neck_end = underside_z_mm

    # Main taper: centerline fixed so traces on the same face stay parallel.
    taper_pts_left = []
    taper_pts_right = []
    for i in range(num_taper_pts):
        ...
        taper_pts_left.append([x_far - hw, y_face, zc])
        taper_pts_right.append([x_far + hw, y_face, zc])

    # Bend section: width stays narrow, centerline moves toward antenna-side feed offset.
    bend_pts_left = []
    bend_pts_right = []
    hw_narrow = 0.5 * w_near
    for i in range(1, num_bend_pts):
        p = float(i) / float(max(1, num_bend_pts - 1))
        zc = z_taper_end + p * (z_bend_end - z_taper_end)
        xc = x_far + p * (x_near - x_far)
        bend_pts_left.append([xc - hw_narrow, y_face, zc])
        bend_pts_right.append([xc + hw_narrow, y_face, zc])

    # Final neck section: narrow and straight, aligned with the feed-transition end.
    neck_pts_left = []
    neck_pts_right = []
    for i in range(1, num_neck_pts):
        p = float(i) / float(max(1, num_neck_pts - 1))
        zc = z_bend_end + p * (z_neck_end - z_bend_end)
        neck_pts_left.append([x_near - hw_narrow, y_face, zc])
        neck_pts_right.append([x_near + hw_narrow, y_face, zc])

    left_edge = taper_pts_left + bend_pts_left + neck_pts_left
    right_edge = taper_pts_right + bend_pts_right + neck_pts_right
...
def create_custom_profile_taper_trace_rot90(name_prefix, ysign, x_face, taper_style,
                                            ...
                                            num_neck_pts=14):
    """
    90-degree rotated version of create_custom_profile_taper_trace().
    The trace lies on a constant-x plane so the copied balun substrate and traces
    form a plus-shaped cross with the original Y-face substrate.
    """
    total = taper_fraction + bend_fraction + neck_fraction
    taper_fraction /= total
    bend_fraction /= total
    ...
    for i in range(num_taper_pts):
        ...
        zc = z3 + p * (z_taper_end - z3)
        hw = width_profile_half(taper_style, p, w_near, w_far)
        taper_pts_left.append([x_face, y_far - hw, zc])
        taper_pts_right.append([x_face, y_far + hw, zc])
    ...
    for i in range(1, num_bend_pts):
        ...
        bend_pts_left.append([x_face, yc - hw_narrow, zc])
        bend_pts_right.append([x_face, yc + hw_narrow, zc])
...
z_source_diff_ohm = 50.0
z_target_antenna_diff_ohm = 200.00

paper_ant_diameter_mm = 290.46
balun_scale = antenna_diameter_mm / paper_ant_diameter_mm

base_balun_dims = {
    "L": 25.821111340632104,
    "L1": 2.58211113406321,
    "L2": 10.32844453625284,
    "S1": 0.516422226812642,
    "S2": 1.6009089031191903,
    "w1": 0.33567444742821734,
    "w4": 6.14542449907044,
    "w5": 15.492666804379262,
}
base_balun_dims["Ltaper"] = 12.910555670316052

# First-pass dimensional retune for the RO4003C-like board and a higher
# antenna-side impedance target (50 -> 200.00 ohm).
ro4003c_length_scale = 1.05
ro4003c_width_scale = 0.96
ro4003c_narrow_width_extra_scale = 0.70
ro4003c_spacing_scale = 1.15

balun_dims = {
    "L": base_balun_dims["L"] * ro4003c_length_scale,
    "L1": base_balun_dims["L1"] * ro4003c_length_scale,
    "L2": base_balun_dims["L2"] * ro4003c_length_scale,
    "S1": base_balun_dims["S1"] * ro4003c_spacing_scale,
    "S2": base_balun_dims["S2"] * ro4003c_spacing_scale,
    "w1": base_balun_dims["w1"] * ro4003c_width_scale * ro4003c_narrow_width_extra_scale,
    "w4": base_balun_dims["w4"] * ro4003c_width_scale,
    "w5": base_balun_dims["w5"] * ro4003c_width_scale,
}
balun_dims["Ltaper"] = balun_dims["L"] - balun_dims["L1"] - balun_dims["L2"]
...
x_top_offset_mm = 0.5 * (balun_dims["w4"] + balun_dims["S2"])
x_bot_offset_mm = 0.5 * (balun_dims["w1"] + balun_dims["S1"])

# Geometry intent:
#   x_bot_offset_mm = narrow pair spacing at the antenna-center side
#   x_top_offset_mm = wide pair spacing at the far end of the balun board
# User-adjusted convergence spacing at the antenna-side neck region.
# Desired clear gap between the two balun microstrips on the same large face
# at the end of the bend / neck region, to better align with the via feed lines.
desired_face_neck_gap_mm = 3.125
x_near_offset_mm = 0.5 * (desired_face_neck_gap_mm + balun_dims["w1"])

# Slightly increase the spacing of the traces over the main balun-body region
# so the exponential side does not appear pinched/bent and there is more room
# between the two pairs on the large faces.
extra_far_pair_spacing_mm = 1.75
x_far_offset_mm = x_top_offset_mm + extra_far_pair_spacing_mm

balun_substrate_material_name = "RO4003C_balun"
balun_substrate_er = 3.55
balun_substrate_tand = 0.0027
ensure_balun_substrate_material(balun_substrate_material_name, balun_substrate_er, balun_substrate_tand)
# Small clearance so the balun substrate sits flush just below the antenna substrate
# without intersecting the antenna substrate or the underside circular via pads.
balun_substrate_gap_mm = 0.036  # retained only as a reported clearance value
...
balun_board_bottom_z_mm = -28.723166
balun_board_top_z_mm = balun_board_bottom_z_mm + balun_board_height_mm
balun_substrate_gap_mm = antenna_bottom_z_mm - balun_board_top_z_mm
...
balun_front_left_lin_obj = create_custom_profile_taper_trace(
    "balun_front_left_lin", -1, front_face_y, "lin",
...
balun_rot90_right_lower_lin_obj = create_custom_profile_taper_trace_rot90(
    "balun_rot90_right_lower_lin", -1, right_face_x, "lin",
    z_top, z3, x_far_offset_mm, x_near_offset_mm, balun_dims, underside_z_mm
)
...
if balun_front_left_lin_obj and via_to_balun_west_obj and via_pad_objs.get("west") and via_cylinder_objs.get("west"):
    unite_keep_first([balun_front_left_lin_obj.name, via_to_balun_west_obj.name, via_pad_objs["west"].name, via_cylinder_objs["west"].name])
...
if balun_rot90_right_lower_lin_obj and via_to_balun_south_obj and via_pad_objs.get("south") and via_cylinder_objs.get("south"):
    unite_keep_first([balun_rot90_right_lower_lin_obj.name, via_to_balun_south_obj.name, via_pad_objs["south"].name, via_cylinder_objs["south"].name])
...
actual_face_neck_gap_mm = 2.0 * x_near_offset_mm - balun_dims["w1"]
...
print("RO4003C first-pass retune length scale = {:.3f}".format(ro4003c_length_scale))
print("RO4003C first-pass retune width scale  = {:.3f}".format(ro4003c_width_scale))
print("Retuned balun L      = {:.4f} mm".format(balun_dims["L"]))
print("Retuned balun Ltaper = {:.4f} mm".format(balun_dims["Ltaper"]))
print("Retuned balun w1     = {:.4f} mm".format(balun_dims["w1"]))
print("Retuned balun w4     = {:.4f} mm".format(balun_dims["w4"]))
print("Antenna-side face neck gap on each active balun face = {:.4f} mm".format(actual_face_neck_gap_mm))
print("Active via mapping: west->front_left_lin, east->back_right_klopf, south->rot90_right_lower_lin, north->rot90_left_upper_klopf")
print("Requested impedance study target = {:.2f} ohm at the antenna-side balanced feed".format(z_target_antenna_diff_ohm))
print("Exponential active branches replaced with Klopfenstein-style tapers.")

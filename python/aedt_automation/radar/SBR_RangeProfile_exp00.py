...
N = 512 - len(phase_history)
# sample_shift = np.exp(1j*)
phase_history = np.pad(phase_history, (0, N), 'constant')
...
# range_profile = np.fft.ifft(phase_history)
half_range = float(range_val_and_units_str[0]) / 2
...
plt.plot(range_vals, 20 * np.log10(np.abs(np.roll(range_profile, int(len(range_profile) / 2)))))
